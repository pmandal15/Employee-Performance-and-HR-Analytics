...
--- I'll create a simple schema for managing employee data, performance reviews, salary information, and departments.
...
--- We want to see the average salary of employees in each department.
...
--- **Employee Performance by Score**
--- Identify employees who scored below average in performance reviews.
...
--- Display the salary change history for a specific employee.
...
--- Find the top performers (score of 4 or 5) in the last year.
...
--- **Employee Retention: Average Years at Company**
--- Calculate the average tenure (years) of employees in the company.


SELECT 
    AVG(DATEDIFF(CURDATE(), e.HireDate) / 365) AS AverageTenure
FROM 
    Employees e;


--- **Salary Distribution: Employees Earning Above Average**
--- List employees who earn more than the average salary in the company.
...
--- Count the number of employees managed by each manager.
